...
export const createRegistration = async (payload, eventId) => {
    ...
    if (participant) {
        const isRegistered = participant.events.some((participantEvent) => participantEvent.eventName === event.name &&
        ...
        await participantsCollection.findOneAndUpdate({ _id: participant._id }, {
            $push: {
                events: {
    ...
    if (!event.participants.includes(participant._id)) {
        await eventsCollection.findOneAndUpdate({ _id: eventId }, {
