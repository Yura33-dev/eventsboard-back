...
async function fetchData(country, pages) {
    const type = 'event';
    let parsePages = 0;
    ...
    let result = [];
    console.log('Fetching events from 3rd API is running... Please, wait a little. Results will display in console');
    try {
        const requests = [];
...
async function getTotalPagesForCountry(country) {
    try {
        const response = await fetch(`https://app.ticketmaster.com/discovery/v2/events.json?type=event&countryCode=${country}&page=0&includeFamily=no&apikey=${apiKey}`);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
function transformData(data) {
    ...
    const transformedData = data.map((event) => {
        ...
        return {
            ...
            imageUrl: event.images?.find((image) => image.width === 640 && image.height === 360)?.url,
        };
    });
    ...
    const eventsWithDates = dataWithoutDuplicates.filter((event) => event.humanDate !== 'Unknown');
    return eventsWithDates;
}
const launchFetch = async (country, pages) => {
    ...
    try {
        ...
        console.log(`Events from 3rd API have been saved in database!. Number of events in DB: ${transformedData.length}`);
    }
    catch (e) {
    ...
    for (const participant of participantsWithEvents) {
        for (const event of participant.events) {
            ...
            if (eventToUpdate) {
                await eventsCollection.updateOne({ _id: eventToUpdate._id }, { $addToSet: { participants: participant._id } });
            }
        }
    }
};
